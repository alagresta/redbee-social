(function() {
  ...
  function TweetService($http, $q) {
    ...
    service.PoolTweetsByUser = PoolTweetsByUser;
    return service;


    function TweetsByUser(id, page) {
      return $http.get('/api/usersTweets/' + id + '/' + page).then(handleSuccess, handleError('Error getting user by tweets'));
    }


    function PoolTweetsByUser(id) {
      return $http.get('/api/poolUsersTweets/' + id).then(handleSuccess, handleError('Error pooling  tweets'));
    }
